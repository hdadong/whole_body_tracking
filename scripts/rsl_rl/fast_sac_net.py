"""FastSAC networks (ported from holosoma) for the IsaacLab whole_body_tracking env.

Faithful copy of holosoma's `agents/fast_sac/fast_sac.py` with the holosoma-only
machinery removed:
  * `obs_indices`/`obs_keys`/`process_obs` slicing -> nets take a flat `n_obs:int`
    (IsaacLab observations are a single flat tensor: policy=101, critic=197);
  * CNN variants dropped (no perception input here).

The full FastSAC algorithm design is preserved unchanged:
  * tanh-squashed Gaussian Actor (pyramid MLP + LayerNorm + SiLU, zero-init heads);
  * C51 distributional critic (`DistributionalQNetwork.projection` bilinear proj);
  * Q-network ensemble (`Critic`), `get_value = sum(probs * q_support)`;
  * per-joint `action_scale` buffer so `action = tanh(mean) * action_scale + bias`
    maps [-1,1] back to FastSAC's action space (see train_sac_match for the value).
"""
...
class Actor(nn.Module):
    ...
    def get_actions_and_log_probs(self, obs: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.use_tanh:
            ...
            # Jacobian corrections: tanh + per-joint scaling.
